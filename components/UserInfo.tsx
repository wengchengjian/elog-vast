...
export default function UserInfo ({user}: UserInfoProps){
    return <>
            <Space spacing={"loose"} className={cn(["white-back-box",`${styles.user_info}`])} vertical={true} align={"center"}>
                {
                    user.avatar ?
                        <Avatar size={"extra-large"} src={user.avatar} style={{ margin: 4 }}/> :
                        <Avatar size={"extra-large"} style={{ margin: 4 }}>{ user.username }</Avatar>
                }
                <Text>
                ...
                <Paragraph size={"small"} ellipsis={{ rows: 3, expandable: true, collapsible: true, collapseText: '折叠我吧', onExpand: (bool, e) => console.log(bool, e) }}>
                    {user.description}
                </Paragraph>
                <Space spacing={"loose"}>
