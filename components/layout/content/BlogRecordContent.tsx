...
export default function BlogRecordContent() {
  ...
  // 统计归档数据
  useEffect(() => {
    sxios.get("/v1/api/article/records", { params: { type } }).then((res) => {
  ...
  const fetchData = useCallback(
    async (start: number, end: number) => {
      const newArticles: Article[] = await sxios.get(
        "/v1/api/article/queryRecordArticle",
        {
          params: {
            type,
            start,
      ...
      let newData = articles.get(date ?? moment().format("yyyy-MM")) ?? [];
      ...
      newData = newData.sort((a, b) => (a.createTime > b.createTime ? 1 : 0));
    ...
    [articles, date, loadedRowsMap, loadingRowCount, type, transformDate]
  );

  const handleInfiniteOnLoad = useCallback(
    async ({ startIndex, stopIndex }: IndexRange, loadLimit: number) => {
  ...
  const renderItem = useCallback(
    ({
      ...
      const item = (articles.get(date ?? moment().format("yyyy-MM")) ?? [])[
        index
      ];
      if (!item) {
        return;
          ...
          <Timeline>
            <Collapse accordion>
              {recordArticles.map((record) => {
                return (
                  <Timeline.Item
                    key={record.archiveDate.toString()}
                    ...
                    >
                      ...
                      >
                        <InfiniteLoader
                          ...
                          loadMoreRows={(range: IndexRange) =>
                            handleInfiniteOnLoad(range, record?.total ?? 0)
                          }
